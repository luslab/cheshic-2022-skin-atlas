...
input <- list.files(data_path, full.names = TRUE)
seurat_list <- lapply(input, function(x) readRDS(x))
seurat_all <- merge(x = seurat_list[[1]], y=seurat_list[-1], add.cell.ids = names(seurat_list), project = opt$id)
